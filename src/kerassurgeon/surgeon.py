...
class Surgeon:
    ...
    def add_job(self, job, layer, *, channels=None, new_layer=None, node_indices=None):
        ...
        # Check that all of the selected nodes are in the layer
            ...
            raise ValueError(
                "One or more nodes specified by `layer` and "
    ...
    def _delete_layer(self, node, inputs, input_masks):
        ...
        if len(inputs) >= 2:
            raise ValueError(
                "Cannot insert new layer at node with multiple " "inbound layers."
            )
        ...
        self._replace_tensors[deleted_layer_output.name] = (inputs, input_masks)
    ...
    def _insert_layer(self, node, inputs, input_masks, new_layer=None):
        ...
        input_masks = utils.single_element(input_masks)
        self._replace_tensors[old_output.name] = (new_output, input_masks)
    ...
    def _replace_layer(self, node, inputs, input_masks, new_layer=None):
        ...
        input_masks = utils.single_element(input_masks)
        self._replace_tensors[replaced_layer_output.name] = (new_output, input_masks)

    def _delete_channels(
        self, node, inputs, input_masks, channels=None, layer_name=None
    ):
        ...
        if len(set(channels)) == getattr(old_layer, utils.get_channels_attr(old_layer)):
            self._replace_tensors[old_layer_output.name] = (None, new_delete_mask)
            return None
        ...
        self._replace_tensors[old_layer_output.name] = (new_output, new_delete_mask)
    ...
    def _apply_delete_mask(self, node, inbound_masks):
        ...
        if any(mask is None for mask in inbound_masks):
            inbound_masks = [
                ...
                if inbound_masks[i] is None
                else inbound_masks[i]
        ...
        if layer_class == "InputLayer":
            ...
            weights = [
                np.delete(w, channel_indices, axis=-1) for w in layer.get_weights()
            ]
            ...
            raise ValueError(
                '"{0}" layers are currently ' "unsupported.".format(layer_class)
            )
    ...
    def _delete_channel_weights(self, layer, channel_indices):
        ...
        if layer.__class__.__name__ == "SimpleRNN":
            ...
            weights = [
                np.delete(w, channel_indices_gru, axis=-1) for w in layer.get_weights()
            ]
            weights[1] = np.delete(weights[1], channel_indices, axis=0)
            ...
            weights = [
                np.delete(w, channel_indices_lstm, axis=-1) for w in layer.get_weights()
            ]
            weights[1] = np.delete(weights[1], channel_indices, axis=0)
        else:
            weights = [
